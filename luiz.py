...
class Usuario:
    def __init__(self, nome, senha, tipo, email, cpf, endereco):
        self.nome = nome
        self.senha = senha
        self.tipo = tipo
        self.email = email
    ...
    def login(self):
        db = Database()
        db.cursor.execute("SELECT * FROM usuarios WHERE nome=? AND senha=? AND tipo=? AND email=? AND cpf=? AND  endereco=? ", 
    ...
    def cadastrar(self):
        ...
        try:
            db.cursor.execute("INSERT INTO usuarios (nome, senha, tipo) VALUES (?, ?, ?, ?, ?)", 
                             (self.email, self.cpf, self.nome, self.senha, self.tipo))
            db.conn.commit()
